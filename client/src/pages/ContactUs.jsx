  ...
  return (
    <div className="relative">
      ...
      <Component
        ...
        className={`w-full pl-12 pr-4 py-4 border-2 rounded-xl transition-all duration-300 bg-white text-black ${
          error
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (serverMessage) {
        ...
        toast.error("Contact endpoint not found (server may be offline).");
      } else if (status === 401 || status === 403) {
        toast.error("Unauthorized. Please check your credentials or CORS settings.");
      } else if (err?.message && err.message.includes("Network Error")) {
        toast.error("Network error: cannot reach server. Is the backend running?");
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 text-white py-12 px-4">
      <div className="max-w-2xl mx-auto relative">
        ...
        <div className="text-center mb-12 animate-fadeIn">
          ...
          <p className="text-lg text-white max-w-md mx-auto">Have questions or need assistance? We'd love to hear from you.</p>
        </div>

        {/* Form */}
        <form onSubmit={handleSubmit} className="bg-gradient-to-br from-purple-900 to-slate-900 rounded-3xl shadow-xl p-8 animate-slideUp">
